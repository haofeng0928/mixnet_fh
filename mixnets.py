...
def MixNetBlock(input_filters, output_filters,
    ...
    def block(inputs):
        # Expand part
        # a = expand_kernel_size, e = expand_ratio
        if expand_ratio != 1:
            x = GroupedConv2D(
                filters,
        ...
        # se = se_ratio
        # layers.Multiply()([x, inputs])
        if has_se:
            x = SEBlock(input_filters, se_ratio, expand_ratio,
                        relu_activation,
                        data_format)(x)

        # p = project_kernel_size
        # layers.Conv2D(splits[i], kernels[i])
        # output phase
        x = GroupedConv2D(
            output_filters,
...
def MixNet(input_shape,
    ...
    x = inputs
    x = GroupedConv2D(
        filters=round_filters(stem_size, depth_multiplier,  # 16
